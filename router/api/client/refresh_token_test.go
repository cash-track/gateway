...
func TestRefreshTokenOk(t *testing.T) {
	...
	assert.Equal(t, fmt.Sprintf("%s%s", endpoint, refreshURI), mock.GetRequest().URI().String())
	...
	assert.Equal(t, fmt.Sprintf("Bearer %s", oldRefreshToken), string(mock.GetRequest().Header.Peek(headers.Authorization)))
	assert.Equal(t, fmt.Sprintf(`{"accessToken":"%s"}`, oldAccessToken), string(mock.GetRequest().Body()))
